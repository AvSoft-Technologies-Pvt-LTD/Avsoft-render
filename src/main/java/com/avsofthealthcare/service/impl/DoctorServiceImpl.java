...
public class DoctorServiceImpl implements DoctorService {
	...
	@Override
	@Transactional
	public String register(DoctorRegisterRequest request, MultipartFile photo) throws IOException {
		try {
...
//				String baseName = request.getFirstName().replaceAll("\\s+", "_") + "_" + savedUser.getId() + "_v1";
//				String extension = FilenameUtils.getExtension(photo.getOriginalFilename());
//				String fileName = baseName + "." + extension;
//				photoPath = fileStorageService.uploadSingleFileWithName(photo, "doctors/photos", fileName);
//			}
...
//			doctorRepository.save(doctor);
			...
			ex.printStackTrace();
			throw new IOException("Doctor registration failed: " + ex.getMessage(), ex);
		}
	}

	@Override
	@Transactional
	public DoctorResponseDto update(Long id, DoctorRegisterRequest req, MultipartFile photo) throws IOException {
		...
		// Master references
		Gender gender = genderRepository.findById(req.getGenderId())
				.orElseThrow(() -> new ResourceNotFoundException("Gender not found"));
		...
		if (photo != null && !photo.isEmpty()) {
			...
			String baseName = req.getFirstName().replaceAll("\\s+", "_") + "_" + doctor.getUserId() + "_v" + version;
			String extension = FilenameUtils.getExtension(photo.getOriginalFilename());
			String fileName = baseName + "." + extension;
